...
void load_code(const bus::address& proc, bus::bus_ptr bus, const elf::elfio& executable);
void load_args(const bus::address& proc, bus::bus_ptr bus, const elf::elfio& executable, int argc, char* argv[]);

void load(const bus::address& proc, bus::bus_ptr bus, const std::string& path, int argc, char* argv[]) {
...
void load_code(const bus::address& proc, bus::bus_ptr bus, const elf::elfio& executable) {
    ...
    for (int i = 0; i < executable.sections.size(); ++i) {
        ...
        if (psec->get_type() == SHT_PROGBITS) {
            ...
            std::vector<value_type> data(reinterpret_cast<const value_type*>(psec->get_data()),
                                         reinterpret_cast<const value_type*>(psec->get_data() + psec->get_size()));
            ...
            if (!std::equal(data.begin(), data.end(), mem.begin())) {
                throw memory_error(proc + mismatch_index(data.begin(), data.end(), mem.begin()));
            }
        }
    }
...
boost::tuple<unsigned int, unsigned int> find_symbol(const elf::elfio& executable, const std::string& name);
    ...
    for (int i = 0; i < executable.sections.size(); ++i) {
        ...
        if (psec->get_type() == SHT_SYMTAB) {
            ...
            for (unsigned int j = 0; j < symbols.get_symbols_num(); ++j) {
                ...
                unsigned char other;
                symbols.get_symbol(j, symbol_name, value, size, bind, type, section_index, other);
                if (symbol_name == name) {
