...
int main(int argc, char* argv[]) {
    try {
        ...
        po::options_description desc("Usage: " + program_name + " [options] address");
        std::string address;
        std::size_t size;
        std::size_t sobuffsize;
        desc.add_options()
            ("help,h", "produce help message")
            ("address,a", po::value<std::string>(&address), "board address, host[:port], default port is 3002")
            ("size,s", po::value<std::size_t>(&size)->default_value(1024), "buffer size in bytes, default: 1024")
            ("bsize,b", po::value<std::size_t>(&sobuffsize)->default_value(4*1024*1024), "socket buffer size in bytes");
        ...
        std::string port = (pos == std::string::npos ? "3002" : address.substr(pos + 1));
        ...
        if (ec) {
            ...
            if (static_cast<std::size_t>(option.value()) != sobuffsize) {
                std::cerr << "warning: socket buffer size is "
                          ...
                          << "change /proc/sys/net/core/rmem_max to fix this warning" << std::endl
                          << "e.g: sudo echo 4194304 > /proc/sys/net/core/rmem_max" << std::endl;
            }
        }
